"termerr" <- function(nodenum,summ.tab,nterms,imp)
  {
    ...
    # If send root node to this function, returns impurity of NA

    rownum <- which(summ.tab[,1]==nodenum)
    if (is.na(summ.tab[rownum,2])){
      nterms <- nterms + 1
      if(sum(summ.tab[,5]==nodenum,na.rm=T)==1){
      ...
      imp <- imp + as.numeric(summ.tab[rowpar,colpar])
    }
    else{
      node.l <- summ.tab[rownum,5]
      tmp.l <- termerr(node.l,summ.tab,nterms,imp)
      nterms <- tmp.l$nterms
      imp <- tmp.l$imp
      
      node.r <- summ.tab[rownum,7]
      tmp.r <- termerr(node.r,summ.tab,nterms,imp)
      nterms <- tmp.r$nterms
      imp <- tmp.r$imp
    }
